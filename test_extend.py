...
def test_extend_method():
    ...
    print("This is a dry run - we'll catch any errors but won't actually extend if there are issues")
    
    try:
        result = token_manager.extend_data_source_token(test_data_source_id, 720)  # 12 hours
        ...
        if result["success"]:
            ...
            print(f"   Token lifetime: {result.get('token_lifetime_minutes', 'N/A')} minutes")
            return True
        else:
